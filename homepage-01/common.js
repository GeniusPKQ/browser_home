...
function common_log(txt) { // 日志函数，注意，已经将其他数据类型强制转化成了string
    common_debug === true ? console.info("%c" + txt, "color:#A0A0A0;font-size:12px;") : console_info_func("common.js YES");
}

/*
* 扩展js框架
* */
const kd_jq = "https://apps.bdimg.com/libs/jquery/2.1.4/jquery.min.js"   +'?'+time_milli(); // jq框架
...
function getNowDate() {
    ...
    var currentdate = [year + sign1 + month + sign1 + day, hour + sign2 + minutes + sign2 + seconds, week, timestamp, time_milli(), year + sign3 + month + sign3 + day];
...
/*
* update 2018-08-15
* 由于对函数命名或者参数写法上的更新，导致旧的仍旧在线的函数不能正常使用，
* 所以需要【用老函数名转发一下新函数及新函数带参数的方式】
*
* */
...
function getUrlParam(key) { // 返回值
    common_log('getThisUrlParam("", key)已经接管了getUrlParam(key)');
...
function isWeiXin(txt, status, url){
    common_log(txt);
    common_log('user_weixin(status, url)已经接管了isWeiXin(txt, status, url)');
